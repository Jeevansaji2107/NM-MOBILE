...
import { OrderStatusChart, FunnelChart } from '@/components/charts/AnalyticsCharts'
...
import { countLowStock, getLowStockProducts, getStockLevel } from '@/utils/stockAlerts'
...
export default function Dashboard() {
  ...
  const deviceCounts = useMemo(() => countDevicesByType(allDevices), [allDevices])
    ...
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-text-secondary mt-2">
            Showing: <span className="text-primary font-medium">{dateRangeLabel}</span>
          </p>
        </div>
        <Select
          label="Date range"
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <Card className="xl:col-span-2">
          <CardHeader title="Revenue" subtitle={`Trend for ${dateRangeLabel.toLowerCase()}`} />
          <RevenueChart data={analytics.revenueData} />
        </Card>
        <Card>
          <CardHeader title="Order Status Distribution" subtitle="Pipeline for selected period" />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <Card>
          <CardHeader title="Customer Growth" subtitle={`New customers · ${dateRangeLabel}`} />
          <CustomerGrowthChart data={analytics.customerGrowthData} />
        </Card>
        <Card>
          <CardHeader title="Order Funnel" subtitle="Conversion through workflow" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card>
          <CardHeader title="Top Selling Products" subtitle="By revenue in period" />
          <div className="space-y-3">
            {analytics.topProducts.length === 0 ? (
              <p className="text-sm text-text-secondary py-2">No product sales in this period</p>
            ) : (
              analytics.topProducts.slice(0, 5).map((p, i) => (
                <div key={p.name} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-text-secondary">{p.sales} sales</p>
                  </div>
                  <p className="text-sm font-medium text-primary shrink-0">{formatINR(p.revenue)}</p>
                </div>
              ))
            )}
          </div>
        ...
        <Card>
          ...
          <div className="space-y-3">
            {analytics.topCategories.length === 0 ? (
              <p className="text-sm text-text-secondary py-2">No category sales in this period</p>
            ) : (
              analytics.topCategories.slice(0, 5).map((c) => (
                <div key={c.name} className="flex items-center justify-between gap-2">
                  <span className="text-sm text-text truncate">{c.name}</span>
                  <span className="text-sm font-medium text-primary shrink-0">{formatINR(c.revenue)}</span>
                </div>
              ))
            )}
          </div>
        ...
        <Card>
          ...
          <div className="space-y-3">
            {analytics.topDeviceModels.length === 0 ? (
              <p className="text-sm text-text-secondary py-2">No device-linked sales in this period</p>
            ) : (
              analytics.topDeviceModels.slice(0, 5).map((d) => (
                <div key={d.model} className="flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    <p className="text-sm text-text truncate">{d.model}</p>
                    <p className="text-xs text-text-secondary">{d.brand}</p>
                  </div>
                  <span className="text-sm font-medium text-primary shrink-0">{d.sales}</span>
                </div>
              ))
            )}
          </div>
      ...
      <Card>
        <CardHeader title="Top Cities" subtitle="Customer revenue by city in period" />
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {analytics.topCities.length === 0 ? (
            <p className="text-sm text-text-secondary col-span-full py-2">No city data for this period</p>
          ) : (
            analytics.topCities.slice(0, 6).map((c) => (
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-text-secondary">{c.state} · {c.customers} customers</p>
                </div>
                <span className="text-sm font-medium text-primary shrink-0">{formatINR(c.revenue)}</span>
              </div>
            ))
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
            ...
            action={
              <div className="flex items-center gap-2 shrink-0">
                {stockCounts.critical > 0 && (
                  <Badge variant="danger">{stockCounts.critical} critical</Badge>
                )}
                {stockCounts.warning > 0 && (
                  <Badge variant="warning">{stockCounts.warning} low</Badge>
                )}
                <Link to="/products" className="text-xs text-primary hover:underline whitespace-nowrap">
                  View all
                </Link>
              </div>
            }
          ...
          {lowStockProducts.length === 0 ? (
            <p className="text-sm text-text-secondary py-4">All products are well stocked</p>
          ) : (
            <div className="space-y-3">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <AlertTriangle
                        size={16}
                        className={level === 'critical' ? 'text-red-400 shrink-0' : 'text-amber-400 shrink-0'}
                      />
                      <div className="min-w-0">
                        <p className="text-sm text-text truncate">{p.name}</p>
                    ...
                    <Badge variant={level === 'critical' ? 'danger' : 'warning'}>{p.stock} left</Badge>
                  </div>
                )
              })}
            </div>
        ...
        <Card>
          <CardHeader title="Recent Orders" subtitle={`Latest in ${dateRangeLabel.toLowerCase()}`} action={<Link to="/orders" className="text-xs text-primary hover:underline">View all</Link>} />
          <div className="space-y-4">
            {analytics.recentOrders.length === 0 ? (
              <p className="text-sm text-text-secondary py-4">No orders in this period</p>
            ) : (
              analytics.recentOrders.map((order) => (
                <div key={order.id} className="p-3 rounded-lg border border-border">
                  <div className="flex items-center justify-between mb-2 gap-2">
                    <span className="text-sm font-medium text-text truncate">{order.orderNumber}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {activities.slice(0, 6).map((a) => (
              <div key={a.id} className="flex gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-text-secondary">{new Date(a.timestamp).toLocaleDateString()}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <Card>
          <CardHeader title="Recent Customers" subtitle="Latest registered customers" action={<Link to="/customers" className="text-xs text-primary hover:underline">View all</Link>} />
          <div className="space-y-3">
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-text-secondary">{c.totalOrders} orders</p>
                  </div>
                </div>
                <p className="text-sm text-primary shrink-0">{formatINR(c.totalSpent)}</p>
              </Link>
            ))}
          </div>
        </Card>
